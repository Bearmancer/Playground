...
public sealed class MusicMetadataService
{
    ...
    )
    {
        ...
        {
            MusicBrainzRelease? mbRelease = await MusicBrainz.GetReleaseAsync(
                mbSearch.Id,
                includeTracks: true,
    ...
    public async Task<Table?> GetReleaseTableAsync(
        MusicSearchQuery query,
        bool includeCredits = false
    )
    {
        UnifiedRelease? release = await GetReleaseAsync(query, includeCredits);
        return release is null ? null : SpectreTables.BuildReleaseTable(release);
    }
